...
from spiderQQMusic.db.DBUtils import insert_IntoUser
...
class RegisterForm(QtWidgets.QWidget):
    ...
    def setup_ui(self):
        ...
        self.pushButton.clicked.connect(self.ifRegister)  # 点击进入登录
        ...
        self.pushButton_2.clicked.connect(self.returnInitialFrame)  # 点击进入登录
    ...
    def ifRegister(self):
        ...
        if self.lineEdit_3.text():  # 如果有邮箱，就赋值到email中
            email = self.lineEdit_3.text()
        else:
            email = " "
        result = insert_IntoUser(user, password, email)
        if result:
            print("账户存在！不可注册 ", result)
            msg = QMessageBox(QMessageBox.Warning, '失败!', '账户存在！不可注册!')  # 小窗口提示框
            msg.exec_()
        else:
            msg = QMessageBox(QMessageBox.Information, '成功！', '用户注册成功!欢迎：' + user)  # 小窗口提示框
            msg.exec_()

    def returnInitialFrame(self):
        self.close()  # 关闭当前登录窗口
