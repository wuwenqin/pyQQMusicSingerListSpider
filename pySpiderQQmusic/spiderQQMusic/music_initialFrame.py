...
from spiderQQMusic.db.DBUtils import connect_whetherUserExists

# 初始登录界面
from spiderQQMusic.music_registerFrame import RegisterForm
from spiderQQMusic.spider.musicInformationSpider import musicInforSpiderFramInit, Ui_Form
...
class LoginForm(QtWidgets.QWidget):
    ...
    def setup_ui(self):
        ...
        self.pushButton.clicked.connect(self.iflogin)  # 点击进入登录
        ...
        self.pushButton_2.clicked.connect(self.turnToRegister)  # 点击进入注册
    ...
    #   点击判断是否登录，
    def iflogin(self):
        user = self.lineEdit.text()  # 账号
        password = self.lineEdit_2.text()  # 密码
        result = connect_whetherUserExists(user, password)  # 查询用户是否存在，存在即可登录爬虫界面
        if result:
            print("账户存在，欢迎: ", result)
            msg = QMessageBox(QMessageBox.Information, '成功!', '即将进入爬虫界面!')  # 小窗口提示框
            msg.exec_()
            # 进入爬虫页面
            self.Form = QtWidgets.QWidget()
            self.spiderFram=Ui_Form()
            self.spiderFram.setupUi(self.Form)
            self.Form.show()
            self.close()
        else:
            msg = QMessageBox(QMessageBox.Warning, '警告！', '用户不存在!')  # 小窗口提示框
            msg.exec_()

    # 前往注册界面
    def turnToRegister(self):
        # 新窗口
        self.register_Frame = RegisterForm()
